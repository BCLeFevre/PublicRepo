...
contract Dice is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 350 350'><style>.base { fill: white; font-family: serif; font-size: 24px; }   .roll { fill: yellow; font-family: serif; font-size: 36px; text-decoration: underline}</style><rect width='100%' height='100%' fill='black' /><text x='10' y='40' class='base'>';

        parts[1] = getDie(tokenId);

        parts[2] = '</text><text x='10' y='185' class='roll'>You rolled a ';

        parts[3] = rollDie(tokenId);
        ...
        string memory json = string(abi.encodePacked('{"name": "Die #', uint2str(tokenId), '", "description": "Dice are randomized dice for your adventuring stories. Everytime you refresh the metadata, you will get a new rolled number based on your die. Feel free to use Dice in any way you want.", "image_data": "', output, '", "attributes": [ {"trait_type": "Type", "value": "' ,  getDie(tokenId), '"}, {"trait_type": "Roll":, "value": "', parts[3], '"}}]}'));
